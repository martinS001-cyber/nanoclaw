...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      // instanceKey is threaded through explicitly — with multiple bot
      // instances registered, chat IDs from different bots can collide
      // (Telegram private-chat IDs are the user's own account ID, so the
      // same person messaging two bots produces the same platformId), and
      // the two must land in separate messaging_groups rows keyed by instance.
      const existing = getMessagingGroupByPlatform('telegram', platformId, instanceKey);
      if (existing) {
...
/**
 * Registers one Telegram bot instance. `instanceKey` is both the adapter
 * registry key and the messaging_groups.instance value for chats this bot
 * owns — 'telegram' is the original default instance (kept unnamed on the
 * bridge so existing rows/behavior are untouched); any other key names a
 * second (or third...) bot running its own token, side by side.
 */
function registerTelegramInstance(instanceKey: string, envVarName: string): void {
  registerChannelAdapter(instanceKey, {
    factory: () => {
      const env = readEnvFile([envVarName]);
      if (!env[envVarName]) return null;
      const token = env[envVarName];
      ...
      const bridge = createChatSdkBridge({
        ...
        maxTextLength: 4000,
        instance: instanceKey === 'telegram' ? undefined : instanceKey,
      });
      ...
      const wrapped: ChannelAdapter = {
        ...
        async setup(hostConfig: ChannelSetup) {
          const intercepted: ChannelSetup = {
            ...hostConfig,
            onInbound: createPairingInterceptor(botUsernamePromise, hostConfig.onInbound, token, instanceKey),
          };
...
registerTelegramInstance('telegram', 'TELEGRAM_BOT_TOKEN');
registerTelegramInstance('telegram-helper', 'TELEGRAM_BOT_TOKEN_HELPER');
