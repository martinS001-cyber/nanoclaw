...
function makeTelegramAdapter(token: string, channelType: string): ChannelAdapter {
...
registerChannelAdapter('telegram', {
  factory: () => {
    ...
    return makeTelegramAdapter(env.TELEGRAM_BOT_TOKEN, 'telegram');
  },
});

registerChannelAdapter('telegram-2', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT2_TOKEN']);
    if (!env.TELEGRAM_BOT2_TOKEN) return null;
